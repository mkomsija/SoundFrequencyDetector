...
extern void SPI_Transmit(uint8_t toSend);

static uint8_t _x, _y;
static uint8_t _sx=1, _sy=1; // scaling factors
...
void OLED_SetColumn(uint8_t column)
{
	...
	column += 32;
	OLED_Command((SSD1306_SETHIGHCOLUMN | (column >> 4))); // SET_HIGH_COLUMN
	OLED_Command((0x0f & column));        // SET LOW_COLUMN
}
...
{
	  uint8_t hun = num / 100;              // Stores hundreds
    uint8_t ten = (num - hun*100) / 10;   // Stores tens
    uint8_t dig = num - hun*100 - ten*10; // Stores numbers from 0 - 9

    // If tens or hundreds are 0 they are not displayed
    if (hun == 0)
    {
        OLED_Putchar(' ');
    }
